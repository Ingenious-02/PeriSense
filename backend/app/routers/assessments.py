...
from backend.app.models import Assessment, Patient, Notification, User
...
@router.get("", response_model=List[AssessmentResponse], summary="List Clinical Assessments (Authenticated)")
def get_assessments(
...
@router.post("", response_model=AssessmentResponse, summary="Save Assessment Record (Authenticated)")
def create_assessment(
    ...
    if patient:
        ...
        patient.latest_assessment_date = now
        db.commit()
...
@router.get("/{assessment_id}", response_model=AssessmentResponse, summary="Get Assessment By ID (Authenticated)")
